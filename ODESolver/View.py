...
class View:
    def __init__(self, root, controller, x0=0, y0=0.5**0.5, X=3, n=100, gui_app_title="ODE_GUI"):
        # internal parameters
        self.root = root
        self.update_flag = 0
        ...
        img = ImageTk.PhotoImage(Image.open("assets/equation.png").resize((150, 70), Image.ANTIALIAS))
        ...
        img = ImageTk.PhotoImage(Image.open("assets/solution.png").resize((200, 100), Image.ANTIALIAS))
        panel = Label(root, image = img)
        panel.image = img
        panel.place(x=340,y=600)
        self.label = Label(self.root, text="Where c1 = {:}".format(((1-y0**2)*math_exp(x0**2))/(y0**2)))
        self.label.place(x=550, y=670)
    ...
    def _update_callback(self):
        ...
        self.canvas_function_graph.draw()
        self.canvas_error_graph.draw()
        self.label.config(text="Where c1 = {:}".format(((1-self.initial_value_y.get()**2)*math_exp(self.initial_value_x.get()**2))/(self.initial_value_y.get()**2)))
